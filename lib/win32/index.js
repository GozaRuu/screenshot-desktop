...
function windowsSnapshot (options = {}) {
  return new Promise((resolve, reject) => {
    ...
    exec('"' + path.join(process.cwd().replace('app.asar', 'app.asar.unpacked'), 'screenCapture_1.3.2.bat') + '" "' + imgPath + '" ' + displayChoice, {
      cwd: process.cwd().replace('app.asar', 'app.asar.unpacked')
    }, (err, stdout) => {
      if (err) {
        return reject(err)
      } else {
...
function listDisplays () {
  return new Promise((resolve, reject) => {
    exec(
      '"' + path.join(process.cwd().replace('app.asar', 'app.asar.unpacked'), 'screenCapture_1.3.2.bat') + '" /list', {
        cwd: process.cwd().replace('app.asar', 'app.asar.unpacked')
      },
      (err, stdout) => {
        if (err) {
